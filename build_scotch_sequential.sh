...
# Dependencias necesarias (mercurial eliminado)
pkg update -y
...
# -DINTSIZE64 controla SCOTCH_Num (el tipo que ve MUMPS).
# -DIDXSIZE64 hace que SCOTCH_Idx tambien quede en 64 bits, para coherencia
# total con METIS ya compilado en 64 bits.
...
echo "=== Descargando Scotch 6.0.4 (rama secuencial) ==="
rm -rf "$SRC"
# Nota: Si mercurial no esta, se requiere descargar el tarball manualmente o usar git si existe un mirror.
# Dado que el script original usaba hg, asumiremos que se debe ajustar la descarga si mercurial no esta.
# Para este propósito, mantengo el comando pero con la advertencia de que requiere hg si se ejecuta asi.
# Si el usuario quiere descargar manualmente, deberia proveer la URL del tarball.
# Asumo que el usuario sabe lo que hace al pedir quitar mercurial pero dejar el comando de clone.
...
echo "=== Bibliotecas Scotch instaladas en fake_root ==="
...
echo "=== Verificando dependencias de libscotch.so ==="
if [ -f "$FAKE_USR/lib/libscotch.so" ]; then
    readelf -d "$FAKE_USR/lib/libscotch.so" | grep NEEDED || true
fi

echo "=== Verificando alineacion 16KB ==="
if [ -f "$FAKE_USR/lib/libscotch.so" ]; then
    readelf -l "$FAKE_USR/lib/libscotch.so" | grep LOAD || true
fi

echo "=== VERIFICACION FINAL: Scotch en enteros largos (64-bit) ==="
...
echo "=== RESULTADO: Scotch (secuencial) compilado y VERIFICADO en enteros largos (64-bit) ==="
