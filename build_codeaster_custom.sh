...
export CPPFLAGS="-I$FAKE_USR/include -I$TMX_PREFIX/include -DIDXTYPEWIDTH=64 -DINTSIZE64 -DAdd_ ${PY_INC_FLAGS} -I$PY_INC_DIR"
export CFLAGS="-fPIC -fPIE -O2 -ffile-prefix-map=$DESTDIR= -Wno-error=implicit-function-declaration -Wno-format $CPPFLAGS"
export CXXFLAGS="-fPIC -fPIE -O2 -ffile-prefix-map=$DESTDIR= -Wno-error=implicit-function-declaration -Wno-format $CPPFLAGS"
export FCFLAGS="-fPIC -fPIE -O2 -ffile-prefix-map=$DESTDIR= -fdefault-integer-8 -fallow-argument-mismatch $CPPFLAGS"
export FFLAGS="$FCFLAGS"
export LDFLAGS="-pie -fuse-ld=lld -Wl,-z,max-page-size=16384 -L$FAKE_USR/lib -L$TMX_PREFIX/lib -landroid-shmem -landroid-posix-semaphore -lpthread -Wl,--allow-shlib-undefined $PY_LD_FLAGS"
...
if p.exists():
    ...
    # Indentation: The original has 4 spaces of indentation.
...
def configure(self):
    ...
    self.env.append_value('LIBPATH', ['$FAKE_USR/lib', '$TMX_PREFIX/lib'])

    # Inyectar LINKFLAGS a la fuerza para proteger la alineación de 16KB de Termux
    termux_ldflags = os.environ.get('LINKFLAGS', '').split()
    self.env.append_value('LINKFLAGS', termux_ldflags)
    self.env.append_value('SHLINKFLAGS', termux_ldflags)
