...
export CXXFLAGS="-std=c++17 -fPIC -fPIE -O2 -ffile-prefix-map=$DESTDIR= $CPPFLAGS"
...
if [ -n "${TFELCFG:-}" ] && [ -x "$TFELCFG" ]; then
  ...
  echo "=== Estándar C++ usado por TFEL ==="
  "$TFELCFG" --cxx-standard || true

  echo "=== Flags de compilación recomendados por TFEL ==="
  "$TFELCFG" --compiler-flags || true
fi
